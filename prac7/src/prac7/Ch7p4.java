...
public class Ch7p4 extends Application {
    ...
    public void mousePressed(MouseEvent evt) {
        ...
        if (evt.isShiftDown() || evt.getButton() == MouseButton.SECONDARY) {
            ...
            squareData.x = x;
            squareData.y = y;
            // Use a random color for the square
            // Choose random RGB components
            squareData.color = Color.color(
                    Math.random(), Math.random(), Math.random(), 0.5 + 0.5*Math.random() );
            squares.add( squareData );
            draw();  // Redraw the picture to show the new square
        }
    }
    ...
    public void mouseReleased(MouseEvent evt) {
        ...
        // Check if the square has moved off the canvas
